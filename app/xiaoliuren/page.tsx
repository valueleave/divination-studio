...
export default function XiaoliurenPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.id) router.push("/xiaoliuren/result/" + data.id);
    } catch (err) {
    ...
    <div className="min-h-screen py-20 px-6">
      <div className="max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-12">
          ...
          <h1 className="text-3xl md:text-4xl font-display tracking-[0.15em] text-foreground mb-3">Xiao Liu Ren</h1>
          <p className="text-gold/70 text-sm font-display tracking-wider mb-4">\u5c0f\u516d\u58ec\u5360\u65ad</p>
